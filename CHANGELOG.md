...
- 超过 500 字符的超长输入/粘贴自动转为文本文件卡片；若溢出前已选择工作空间，文件直接保存到该工作空间，否则保存到应用临时工作空间（`~/.pure/workspace/<session>/`）
- 引擎启动前（预检阶段）手动停止 / ESC 中断后，原始请求会写入会话历史：同一会话内回复「继续」，模型能准确接续之前的任务，不再出现"没有之前对话的上下文记录"
- 「模型响应较慢」提示改为在输出恢复并上屏后停留 1 秒再淡出消失；普通首字等待场景的提示同样会被正确清理，不再滞留到阶段结束
- 中断的会话以工具结果结尾时，其产物卡片在重放后不再丢失
...
- 地图 JSON 容错解析：注释、尾逗号、单引号、无引号 key、包裹文字自动修复，失败时中文友好错误（不再泄露 `JSON Parse error: Unrecognized token '/'`）
- 图表 / Mermaid / SVG 渲染错误统一中文文案，不再透传库内部英文报错
- 内联地图改为静态预览，新增「展开地图」按钮打开全屏交互地图（拖拽 / 缩放 / 标记弹窗 / 点击取坐标）
- 移除 llm-verify 内部验证建议的气泡展示，内部审查结论不再显示给用户
- 思考过程逐字重复自动折叠，并要求模型按用户语言推理与回答
- 删除会话时后台清理该会话的生成/暂存内容（后台线程、不阻塞界面）

## v1.9.14-beta3

**地图瓦片离线缓存 + 地理方向校验 + 合理性审视 + 虚构意图识别**

- 地图瓦片本地磁盘缓存：看过的区域断网也能查看；设置面板新增缓存大小上限与一键清空，并显示当前占用
- 地图多数据源可达性链（OSM / Esri / Carto / 高德 / 腾讯）依次回退，国内环境自动落到高德/腾讯并做 GCJ-02 ↔ WGS-84 坐标对齐
- 地图卡片增高、去掉瓦片署名、标记点与空白处点击弹出经纬度弹窗
- 路线方向程序化地理校验：途经点方位角偏离目的地方向超过 90° 时自动拦截并在卡片上告警
- 常驻「合理性审视」契约：答案/路线/事件序列交付前自检地理、物理、化学、数学、历史规律
- Planner 确定性识别虚构/架空/不用管事实意图，自动跳过合理性审视（不再依赖模型判断）
- 网络调研连续轮数守卫：web 工具连续空结果达到阈值后停止重搜、基于已有证据给出完整答案

## v1.9.14-beta2

**聊天内地图 + 首句加速 + 卡顿修复**

- 新增聊天内地图：模型输出 ````map` 代码块（JSON：markers/route/center）时，聊天中直接渲染可交互的 Leaflet + OpenStreetMap 地图（标记 + 路线折线），适合路线规划、出行/地点类回答
- 修复图片生成/工具执行期间不断弹出「正在思考下一步」：正在执行的工具行自带 spinner 时不再叠加快待卡片，文本流式时也不再反复重触发
- 修复新对话首句响应慢：记忆模型（WASM）改为启动时后台预热，记忆检索加 1.5s 上限，冷启动不再阻塞首 token
- 修复大代码块流式渲染因反复代码高亮导致的界面卡死（>30KB 跳过流式高亮）
- 修复 Esc 中断后项目目录卡片重复显示

## v1.9.14-beta

**大模型慢响应待动效 + 多阶段计划推进修复 + 稳定性**

- 工具卡片调用之间新增「正在思考下一步…」等待动效：工具结果完成或文本流暂停后，若模型迟迟未输出下一步，自动弹出带跳动圆点的等待卡片，避免用户误以为会话卡死
- 修复图片生成/工具执行期间不断弹出「正在思考下一步」的问题：有工具行正在执行时不再叠加快待卡片，文本流式到达时也不再反复重新触发
- 修复多阶段构建计划停在「计划 1」的问题：静态网页项目（无标准测试清单）不再被阶段验证证据卡死；中间阶段交付门禁失败不再中断自动续跑链（仅最终阶段生效）
- 分平台浏览器 User-Agent（Windows/macOS/Linux 各自匹配），Chrome 版本升级到 152
- 修复非推理模型产生空「思考完成」卡片的问题
- **新增聊天内地图**：模型输出 ````map` 代码块（JSON：markers/route/center）时，聊天中直接渲染可交互的 Leaflet + OpenStreetMap 地图（标记 + 路线折线），适合路线规划、出行/地点类回答；CLI 仍按普通代码块展示
- 修复新对话首句响应慢：记忆模型（~80MB WASM）改为启动时后台预热，且记忆检索加 1.5s 上限，冷启动不再阻塞首 token
- 修复大代码块流式渲染时因反复代码高亮导致的界面卡死：超大代码块（>30KB）在流式与最终渲染中跳过 hljs 高亮（保持文本可读）
- 修复 Esc 中断后项目目录卡片重复显示（bool 卡片被渲染两次）

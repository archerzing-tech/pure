...
- CLI 终端线框图渲染：`mermaid`（graph/flowchart）与 `puml` / `plantuml`（活动图/顺序图）围栏代码块不再原样打印源码——完整代码块出现时自动转换为由矩形框、连接线与箭头组成的盒式线框图（box-drawing 字符，CJK 标签自适应宽度）；流式输出即时替换，未闭合的残缺代码块保持原文兜底，CLI 系统提示同步引导模型用 mermaid/puml 表达流程图
- 执行大纲悬浮卡支持拖动重定位：展开列表卡与收起后的圆形胶囊均可按住拖动，位置按会话记忆（重启应用或切换会话后自动恢复，并钳制在窗口内）；拖动时启用指针捕获，普通点击不受影响
- 修复完成状态：计划全部执行完后大纲显示全部勾选而不是停在第一步或中途消失——收尾不再依赖模型是否恰好发出 `## 计划 n 已完成` 标记，而是以回合正常结束 + 真实工具执行为准；提问/暂停/中断轮不会误判完成
- 收起态胶囊改为「当前第 n 步/共 n 步」数字 + 呼吸光圈，取代文字标签与进度文字
- 用户气泡双击全选整条消息文字（会话回放同样支持）；assistant 气泡保持双击复制到剪贴板
- 全链路中止/超时加固：工作区解析、运行时探针、MCP 初始化、工作区扫描等预检均有超时与中止保护，Esc/停止可中断慢启动；卡死的任务分析流不再让界面永远停在流式状态
- 工具输出结构化展示：JSON/YAML 输出与 JSON 型参数自动检测、美化并高亮，不再原样堆文本
- 工作区发现（discoverWorkspace）支持中止信号与单次工具 8 秒超时
- 文生图能力自动适配：接入支持 OpenAI 兼容 `/images/generations` 的模型/供应商（如 OpenAI 预设的 gpt-image-1 / dall-e-3，或模型名含 gpt-image/dall-e/cogview/flux 等）时，图片请求自动切换为真实图片渲染——模型调用 generate_image 工具，界面以 ChatGPT/Gemini 式 `<img>` 卡片展示（多图并排、点击放大、原生保存对话框下载），图片随会话持久化、回放同样显示；模型不支持时保持原有 SVG 输出契约作为自动兜底，工具失败也会回退 SVG
- 自定义供应商设置新增「图片生成（文生图）」开关与图片模型名；图片数据只经侧通道送达界面，不会把 base64 图片塞进模型上下文

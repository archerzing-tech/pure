...
- 同一轮里工具卡和子 Agent 卡不再挤成一排：先 sys_info 查位置、再并行派三个调研 Agent 时，以前四张卡一字排开，看不出谁是前置、谁是并行。现在工具卡单独一行，并行的 Agent 卡另起一行并排——前置步骤和并行兵团一目了然。分类不重排时间顺序：模型若是先派 Agent 后调工具，工具卡照样显示在下面。会话回放用同一条规则。
- 并行 Agent 一行最多三个：派四个以上时，多出的在 Agent 自己的网格里换行继续，仍是并行关系，绝不会和后面的工具卡混到一行；工具卡也不受影响（原本几列就几列）。

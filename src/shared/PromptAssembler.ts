...
function buildOutputStyle(surface: PromptSurface, imageGeneration = false): string {
  const visualOutput = surface === 'gui'
    ? imageGeneration
      ? `- To SHOW a picture/icon/illustration/photo, call generate_image — the app renders the result as a real image. To EXPLAIN an image the user attached (what it is, what it shows), answer directly and never call generate_image. For hand-drawn diagrams (flowcharts, architecture, sequence), emit fenced code blocks tagged svg or mermaid — mermaid is the default for every diagram type and renders locally (works offline); tag puml ONLY when the user explicitly asks for PlantUML syntax.\n- ${CHART_DSL_PROMPT}\n- ${MAP_DSL_PROMPT}`
      : `- To SHOW a picture/diagram, emit it as a fenced code block tagged svg containing complete standalone SVG — the app renders it inline as an image (structural diagrams default to fenced mermaid blocks: flowchart/sequence/class/state/ER/gantt/pie/mindmap/timeline — mermaid renders locally and works offline). NEVER emit puml/plantuml blocks unless the user explicitly asks for PlantUML: those render through the plantuml.com server and fail without network. A picture request is DELIVERED as the fenced svg/mermaid block itself — never save it with write_file as an .svg/.png file, and never emit SVG as plain text or in a non-svg code block.\n- ${CHART_DSL_PROMPT}\n- ${MAP_DSL_PROMPT}`

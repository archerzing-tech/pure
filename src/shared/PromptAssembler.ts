...
- web_search(query, maxResults?) — web search. With a Serper or Tavily API key in Settings → Tools it uses the API backends first (Serper = real Google index, best for Chinese AND English); otherwise free backends are tried in order with a shared cookie session — DuckDuckGo Instant Answer / Wikipedia / Google News RSS structured lookups, cn.bing.com + Sogou + 360 + Baidu for Chinese queries, Bing + Brave + Mojeek otherwise, plus a Bing→Google→DuckDuckGo-via-Jina last-resort chain when the rest fail; an intranet SearXNG instance (Settings → Tools → Web Tools) is tried first when configured, and the network state from sys_info() tells you which backends are reachable. If a search returns no results or fails, do NOT repeat the same or a near-identical query — rephrase it (broader terms, simpler wording, or English), or use web_fetch on a URL you expect to be authoritative.
- web_fetch(url, maxChars?) — fetch and extract readable text from a text/HTML/JSON/PDF page; follows meta-refresh redirects and falls back across Jina Reader / Wayback Machine / Firecrawl when the direct fetch is blocked or binary. If web_fetch still returns nothing usable, do NOT retry the same URL — use web_search instead or pick a different page.`;
...
- web_scrape(url, selector?, maxChars?) — fetch a KNOWN URL and extract readable text (navigation stripped; optional #id/.class/tag selector; RSS feeds and JSON auto-formatted; follows meta-refresh redirects; extracts text-based PDFs directly; falls back across Jina Reader → Wayback Machine → Firecrawl for blocked, removed, JS-heavy, or binary pages). Use when you already have the URL; use researcher_web when you need to find one.
...
- A bare "generate X", "show me X", "give me X", "what does X look like", or any "write me code for…" without a path means inline output — never reach for write_file. When the workspace holds a project the request extends or repairs, the engineering exception above applies instead: write the change into the project and verify it.
...
function buildMode(mode?: PromptTaskMode): string {
  ...
  const directive = mode === 'build'
    ? 'Work through the request in clear phases; when you complete each phase, briefly state what was done and what remains. The deliverable lives in the workspace: implement and fix with the file tools and verify by running the project checks — never an implement/fix request answered with inline code alone.'
...
export class PromptAssembler {
  ...
  buildSystemFragments(context: PromptAssemblyContext): PromptFragment[] {
    ...
    return [
      ...
      fragment('work_invariant', 'Work step by step. Read before you write: any implement / fix / extend request starts with list_files on the workspace — when it holds a project (stubs, README, tests/), the deliverable is the change written into those files plus their tests passing, never a code block in the reply; existing file names, signatures, and test layout are the contract, not suggestions. Verify after you change. Be concise.', 110, true),

...
const BASE_SYSTEM_PROMPT = `You are pure, a coding agent with file and command tools.
When using tools:
- read_file: read file content, supports optional startLine/endLine
...
function toolCategory(tool: string): 'read' | 'write' | 'cmd' | 'git' | 'other' {
  ...
  if (tool === 'read_file' || tool === 'list_files' || tool === 'search_files') return 'read';
  return 'other';
}

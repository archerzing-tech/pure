...
const PLAN_STAGE_PROTOCOL = `Use this strict stage protocol for every top-level plan item. The control lines below are machine-readable progress marks for the interface — the user never reads them as prose, so write them exactly as given and keep every sentence you write around them natural. Before any tool call or file change for plan n, first write one standalone control line exactly \"## 计划 n：<阶段名称>\" and then one or two natural sentences telling the user what this stage is about and what you are going to do. Do not call tools before that line. After the work and its meaningful verification are actually complete, write a standalone control line exactly \"## 计划 n 已完成\" and briefly tell the user what this stage delivered. These progress marks are state changes, not a generic progress estimate; they do not dictate execution granularity. Do not start the next stage, call its tools, or claim the whole plan is complete before the current stage's completion line. If a stage needs user input, explain what is missing and pause without emitting its completion line. Never emit a stage marker as a preview or example. A plan is introduced exactly ONCE per conversation: when you resume, repair, or continue work inside an already-introduced plan, do NOT write its start line again (no repeated 「## 计划 n：…」 lines, and no repeated completion lines either) — the interface already shows the plan card, so one introduction at the start and one completion at the end is all it needs. For a plan item that carries a visible Todo list (substeps), drive that list with the same discipline: before starting substep k, write a standalone control line exactly 「### 子步骤 k：<子步骤名称>」 (only for substeps not yet marked), and once that substep's work is actually done and verified, write a standalone control line exactly 「### 子步骤 k 已完成」 (an optional 「/总数」 after the number is accepted). The interface advances the Todo list only in numeric order, so emit the marks in order and never mark a substep done before its work is finished. Skip substep marks entirely for an atomic plan item that has no Todo list.`;
...
export function formatPlanForPrompt(plan: Plan, projectBuild = false, approved = false): string {
  ...
  const execution = approved
    ? 'Use this plan as a flexible guide — you are its author. The user has already approved this plan, so start executing immediately: begin the most appropriate next action with real tool calls in this same response — do not wait for another approval, and do not restate the request or re-print the plan list (the plan card on screen already shows the steps). Verify the result. When a plan card is active, follow the strict stage protocol below. If the task needs information, ask one natural question and pause until the user answers. In later responses, continue with the next appropriate work and verify meaningful results. Emit `## 计划 n 已完成` when the plan work has actually been completed and verified' + (projectBuild ? ' (for a project build, the LAST plan stage must be the delivery verification pipeline: code review, then typecheck, then unit tests, then e2e/build checks — fix failures and re-run until ALL pass).' : '.') + ' The UI may reflect the completed plan and activate the next plan. When todosRequired=false, treat the step as atomic unless the task itself reveals a reason to split it. Never claim a plan or work item is complete without relevant verification evidence. Use plain language and natural colleague-like sentences around the required markers.'
    : 'Use this plan as a flexible guide. IMPORTANT: this first planning response is a pause point — do not call tools or change files yet. Introduce the plan in your own words — what you emphasize and how you frame it is yours to decide; the steps below are your commitment, not a script to read back. End by telling the user what you recommend starting with and why. Only after the user sends the next message may you begin execution. If the task needs information, ask one natural question and pause until the user answers. In continuation responses, continue with the next appropriate work and verify meaningful results. When a plan card is active, follow the strict stage protocol below. Emit `## 计划 n 已完成` when the plan work has actually been completed and verified' + (projectBuild ? ' (for a project build, the LAST plan stage must be the delivery verification pipeline: code review, then typecheck, then unit tests, then e2e/build checks — fix failures and re-run until ALL pass).' : '.') + ' The UI may reflect the completed plan and activate the next plan. When todosRequired=false, treat the step as atomic unless the task itself reveals a reason to split it; respect the current execution context. Never claim a plan or work item is complete without relevant verification evidence. Use plain language and natural colleague-like sentences around the required markers.';
  const firstTurn = approved
    ? 'The plan is already approved, so do NOT wait for another go-ahead: start the next appropriate work with real tool calls in this same response. End by reporting what you did and what remains.'
  ...
  // 2026-09-26 拆仪式：不再要求「复述请求 + 完整打印计划清单 + Todo 清单」——
  // 计划卡就是那份清单，聊天里再抄一遍正是用户点名的固定腔。
  return `\n\n## 整体安排\n${steps}\n${execution}${parallelRule}\n\n${PLAN_STAGE_PROTOCOL}\n\nThe plan card already shows the steps — never re-print the full plan list in prose; say what you are doing and why as you go. ${firstTurn} Finish by summarizing what changed.`;
}
...
export function formatPlanPauseMessage(plan: Plan): string {
  ...
  return `这个任务一口气做完容易翻车，我拆成了几步，先把整体安排给你过目：\n\n${planLines}\n\n我准备先做第 1 项「${first?.action ?? '当前阶段'}」。${todoLines ? `这一阶段拆成这几件小事：\n${todoLines}` : ''}\n\n安排先放这儿——有想调整的直接说；觉得没问题就回我一声，我按这个顺序开工。`;
}
...
export function formatPlanContinuation(plan: Plan, currentPlan: number, currentTodo: number, projectBuild = false): string {
  ...
  return `\n\n<plan_continuation>\n这是一个已经开始的复杂任务，不要重新生成计划，也不要从头开始。\n\n当前总计划：\n${planLines}\n\n当前阶段 Todos（阶段 ${currentPlan}）：\n${todos || '当前阶段没有拆分 Todo，请直接处理这一阶段。'}\n\n继续规则：把这里的计划和 Todo 当作当前上下文，根据实际依赖选择下一步工作。开始任何工具调用之前，用你自己的方式让用户自然地接上进度（做到哪了、从哪里继续）——不要套固定开场白，然后再继续。可以合并紧密相关的小项，也可以在需要信息时先提问并暂停。阶段标记：只有尚未标记过的计划才需要输出「## 计划 n：<正在做什么>」开始行；当前计划如果在之前的对话里已经标记过（计划卡与进度条都在走），不要再输出开始行，直接继续干活；真正进入一个新计划（编号变化）时才标记。当前阶段真实完成并验证后，再单独输出「## 计划 n 已完成」，并简要说明结果；同一计划只标记一次开始、一次完成，修复/续跑轮绝不重复输出这两行。编号与上面的列表保持一致，界面靠这两行标记推进进度。开始行没写就不要执行，完成行没写就不要进入下一计划。如果当前阶段带 Todo 列表，同样按序标记子步骤：仅在子步骤尚未标记过时输出「### 子步骤 k：<名称>」，该项真实完成后再单独输出「### 子步骤 k 已完成」（数字后可选「/总数」）；没做完就不要标完成。进展用自然的话说，Todo 勾选标记只在有帮助时用。${projectBuild ? '项目级交付仍需提供真实验证证据。' : ''}\n</plan_continuation>`;
}
